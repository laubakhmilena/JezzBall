(() => {
  ...
  const createTopUi = () => `
    <header class="top-ui" aria-label="Верхняя панель">
      ...
      <div class="resource-strip" aria-label="Ресурсы">
        <div class="resource-pill lives-pill" aria-label="Жизни"><span class="resource-icon" aria-hidden="true">♥</span><span data-resource="lives">5</span><span class="resource-plus" aria-hidden="true">+</span></div>
        <div class="resource-pill coins-pill" aria-label="Монеты"><span class="resource-icon" aria-hidden="true">●</span><span data-resource="coins">1280</span><span class="resource-plus" aria-hidden="true">+</span></div>
        <button class="resource-pill achievement-button" type="button" data-action="achievements" aria-label="Достижения"><span class="resource-icon" aria-hidden="true">★</span><span data-resource="achievements">0</span></button>
      </div>
      ${createIconButton("shop", "Магазин", "M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2Zm10 0c-1.1 0-1.99.9-1.99 2S15.9 22 17 22s2-.9 2-2-.9-2-2-2ZM7.16 14.26c-.75 0-1.41-.41-1.75-1.03L2 6.2V5h3.21l.94 2h12.9c.75 0 1.24.78.92 1.45l-2.42 5.05A2 2 0 0 1 15.74 14H8.1l-1.1 2h12v2H7c-1.52 0-2.48-1.63-1.75-2.96l1.03-1.86-.12-.24ZM7.1 9l1.42 3h7.22l1.44-3H7.1Z", "shop-button")}
    </header>
  `;
  ...
  const createChapterItem = (chapter) => {
    ...
    const lockMarkup = isUnlocked ? "" : '<span class="chapter-lock" aria-hidden="true">▣</span>';
    const levelsMarkup = isExpanded ? `
    ...
    return `
      <article class="chapter-card ${isExpanded ? "is-expanded" : ""} ${isUnlocked ? "is-unlocked" : "is-locked"}">
        <button class="chapter-card-head" type="button" data-action="toggle-chapter" data-chapter-id="${chapter.id}" aria-expanded="${isExpanded ? "true" : "false"}">
          <span class="chapter-badge" aria-hidden="true">${chapter.icon}</span>
    ...
    screen.innerHTML = `
      <div class="chapter-scene">
        ...
        <div class="chapter-select">
          <header class="chapter-select-title">
        ...
        <nav class="bottom-nav" aria-label="Нижняя навигация">
          <button type="button" data-action="map"><span aria-hidden="true">▰</span>Карта</button>
          <button class="is-active" type="button" data-action="chapters"><span aria-hidden="true">▥</span>Главы</button>
  ...
  const handleAction = (event) => {
    ...
    if (action === "toggle-chapter") {
      ...
      renderChapterScreens();
      showScreen(getChapterScreenId(selectedChapterId));
      return;
    }
